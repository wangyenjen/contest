...
pii dfs(int n, int m) {
  if (n == 2) {
    if (m == 0) return MP(1, 1);
    else if (m == 1) return MP(1, 2);
    else if (m == 2) return MP(2, 2);
    else return MP(2, 1);
  }
  int t = n / 2;
  int region = m / (t * t);
  pii p2 = dfs(t, m % (t * t));
  if (region == 0) swap(p2.x, p2.y);
  else if (region == 1) p2.y += t;
  else if (region == 2) p2.x += t, p2.y += t;
  else p2 = MP(2 * t - p2.y + 1, t - p2.x + 1);
  return p2;
}
